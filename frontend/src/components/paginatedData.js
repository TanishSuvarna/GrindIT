...
const PaginatedData = (query, variable, offset) => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((data) => {
        if (
          (Array.isArray(data.data[variable]) && !data.data[variable].length) ||
          (!Array.isArray(data.data[variable]) &&
            !data.data[variable].userComments.length)
        ) {
          setEmpty(true);
          setLoading(false);
        }
        ...
        setgetBlogs((prevBlogs) => {
          if (Array.isArray(data.data[variable])) {
            ...
            return {
              ...prevBlogs,
              userComments: [
                ...prevBlogs.userComments,
                ...data.data[variable].userComments,
              ],
            };
          }
        });
